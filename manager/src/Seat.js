...
export default function Seat() {
    ...
    const prepareMoveSeat = () => {
      ...
      const moveSeat = async(newTableItem) => {
        ...
        if (isEmpty === true){
            //move table
            console.log('테이블 이동!')
            await API.graphql(graphqlOperation(updateRestaurant, { input: {id: newTableItem.id, tableNumber: newTableItem.tableNumber, storeId: storeId, available: false} }))
            .then(result => {
            console.log('Restaurant updated:', result.data.updateRestaurant);
            fetchData();
            })
    ...
    <div className="container">
      ...
      <Modal open={adminModalvisible} title={`${tableItem.tableNumber}번 테이블`}
        okText="자리 비우기" onOk={() => {releaseTable(); setAdminModalvisible(false)}}
        cancelText="자리 옮기기" onCancel={() => {prepareMoveSeat();}}>
      </Modal>
        <Modal open={fillSeatModalVisible}
            title={`${tableItem.tableNumber}번 테이블을 채우시겠습니까?`}
            okText="예" onOk={() => {reserveTable(); setFillSeatModalVisible(false);}}
            cancelText="아니오" onCancel={() => setFillSeatModalVisible(false)}>
        </Modal>
    </div>
  );
}
